...
import { actionEnter, transactionConstruct, useTokenGetTokenBalances, Networks } from "@stakekit/api-hooks";
...
export function InvestmentModal({ isOpen, handleClose, yieldOption }: InvestmentModalProps) {
	...
	const tokenBalance = balanceQuery.data?.find((balance) => balance.token.symbol === yieldOption.token_symbol);
	...
	}, [requiredChainId, switchChain, toast, yieldOption]);
	...
	const handleInvest = useCallback(async () => {
		...
		try {
			...
			toast({
				...
				description: error instanceof Error ? error.message : "There was an error processing your investment",
				variant: "destructive",
			});
		} finally {
			setIsInvesting(false);
		}
	}, [address, amount, isConnected, processTransactions, toast, yieldOption, handleSwitchChain, needsChainSwitch]);
		...
		<Dialog open={isOpen} onOpenChange={handleClose}>
			<DialogContent className="sm:max-w-lg bg-cream-100 border-2 border-gray-900 rounded-xl shadow-lg">
				<DialogHeader className="space-y-3">
					<DialogTitle className="text-2xl font-bold text-gray-900">Invest in {yieldOption.token_symbol}</DialogTitle>
					<div className="flex items-center gap-3">
						<div className="h-10 w-10 rounded-full bg-yellow-300 flex items-center justify-center border-2 border-gray-900">
							<span className="font-bold text-gray-900">{yieldOption.token_symbol.substring(0, 1)}</span>
						</div>
						<div className="space-y-1">
							<div className="flex items-center gap-2">
								<div className="font-bold text-lg text-gray-900">{yieldOption.provider_name}</div>
								<Badge variant="outline" className={`${getChainColor(yieldOption.token_network)} bg-blue-100 text-blue-800 text-xs px-2 py-0.5 rounded-full border border-blue-300`}>
									{yieldOption.token_network}
								</Badge>
							</div>
							<div className="text-xl font-bold text-orange-500">{formatPercentage(yieldOption.apy * 100)} APY</div>
						</div>
					</div>
				</DialogHeader>
				...
				<div className="grid gap-6 py-6">
					<div className="bg-white rounded-lg p-4 border-2 border-gray-900 shadow-sm">
						<div className="flex items-center justify-between">
							<Label className="text-gray-900 font-medium">Available Balance</Label>
					...
					<div className="bg-white rounded-lg p-4 border-2 border-gray-900 shadow-sm space-y-4">
						...
						<div className="flex items-center gap-3">
							...
							/>
							<span className="font-bold min-w-16 text-right text-gray-900 text-lg">{yieldOption.token_symbol}</span>
						</div>
					</div>

					<div className="bg-white rounded-lg p-4 border-2 border-gray-900 shadow-sm space-y-3">
						<Label className="text-gray-900 font-medium">Percentage of Balance</Label>
					...
					<div className="bg-cream-200 rounded-lg p-4 border-2 border-gray-900 shadow-sm space-y-3">
						<h3 className="font-bold text-gray-900 text-lg">Investment Summary</h3>
						<div className="space-y-2">
							<div className="flex items-center justify-between">
								<span className="text-gray-700 font-medium">Estimated Yearly Yield</span>
								<span className="font-bold text-gray-900">
							...
							<div className="flex items-center justify-between">
								...
								<span className="font-bold text-orange-500 text-lg">{formatPercentage(yieldOption.apy * 100)}</span>
							</div>
						</div>
					</div>
				</div>
				...
				<DialogFooter className="flex gap-3 pt-6">
					<Button variant="outline" onClick={handleClose} disabled={isInvesting} className="flex-1 border-2 border-gray-900 bg-white hover:bg-gray-50 text-gray-900 font-medium">
						Cancel
					</Button>
					<Button
						onClick={handleInvest}
						disabled={isInvesting || isSwitchingChain || !amount || Number.parseFloat(amount) <= 0 || balanceQuery.isLoading}
