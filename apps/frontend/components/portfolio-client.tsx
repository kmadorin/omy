...
export default function PortfolioClient() {
			...
			<div className="w-full max-w-4xl mx-auto">
				<Card className="p-8 bg-cream-100 border-2 border-gray-900 shadow-md">
					<CardContent className="text-center">
						<p className="text-navy">Connect your wallet to view your portfolio.</p>
					</CardContent>
				</Card>
			</div>
		);
	...
	if (isLoading || !data) {
		return null; // Loading is handled at page level
	}
	...
	return (
		<div className="space-y-6">
			<h1 className="text-4xl font-bold text-center text-gray-900">Your Portfolio</h1>
					...
					<CardContent className="space-y-4">
						<div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
							<div>
								...
								<div className="text-3xl font-bold text-navy">${totalUsd.toFixed(2)}</div>
							</div>
							<div>
								<div className="text-sm text-navy font-medium">On-chain Amount</div>
								<div className="text-2xl font-bold text-navy">{totalOnChain.toFixed(2)}</div>
							</div>
							<div>
								<div className="text-sm text-navy font-medium">Average APY</div>
								<div className="text-2xl font-bold text-navy">{(avgApy * 100).toFixed(2)}%</div>
							</div>
						</div>
					</CardContent>
				</Card>
			...
			{data.length === 0 ? (
				<div className="w-full max-w-4xl mx-auto text-center">
					<Card className="p-8 bg-cream-100 border-2 border-gray-900 shadow-md">
						<CardContent>
							<p className="text-navy">No active positions found. Start exploring yield opportunities!</p>
						</CardContent>
					</Card>
				</div>
			) : (
