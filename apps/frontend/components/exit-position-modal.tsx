...
export function ExitPositionModal({
  ...
  const handleExit = useCallback(async () => {
    ...
    try {
      ...
      const successTxs = await processTransactions(
        actionResponse.transactions,
        {
          wallet: address,
      ...
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : "There was an error processing your exit",
        variant: "destructive",
      });
    } finally {
      setIsExiting(false);
  ...
  const exitPercentage =
    (Number.parseFloat(exitAmount || "0") / maxAmount) * 100;
      ...
      <DialogContent className="sm:max-w-lg bg-cream-100 border-2 border-gray-900 rounded-xl shadow-lg">
        <DialogHeader className="space-y-3">
          <DialogTitle className="text-2xl font-bold text-gray-900">
            Exit from {position.provider_name || "Yield Position"}
          </DialogTitle>
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-full bg-yellow-300 flex items-center justify-center border-2 border-gray-900">
              <span className="font-bold text-gray-900">
                {position.token_symbol?.substring(0, 1) || "T"}
              </span>
            </div>
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <div className="font-bold text-lg text-gray-900">
                  {position.provider_name || "Provider"}
                </div>
                <Badge
                  variant="outline"
        ...
        <div className="grid gap-6 py-6">
          <div className="bg-white rounded-lg p-4 border-2 border-gray-900 shadow-sm">
            <div className="flex items-center justify-between">
              ...
              <span className="font-bold text-gray-900">
                {`${maxAmount.toFixed(6)} ${position.token_symbol || "TKN"}`}
              </span>
            </div>
          </div>
          ...
          <div className="bg-cream-200 rounded-lg p-4 border-2 border-gray-900 shadow-sm space-y-3">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="font-bold text-gray-900">
                  {Number.parseFloat(exitAmount || "0").toFixed(4)}{" "}
                  {position.token_symbol || "TKN"}
                </span>
              </div>
              <div className="flex items-center justify-between">
              ...
              <div className="flex items-center justify-between border-t pt-2">
                ...
                <span className="font-bold text-orange-500 text-lg">
                  {formatPercentage(exitPercentage)}
                </span>
              </div>
            </div>
          </div>
        ...
        <DialogFooter className="flex gap-3 pt-6">
          ...
          <Button
            ...
            disabled={
              ...
              isSwitchingChain ||
              !exitAmount ||
              Number.parseFloat(exitAmount) <= 0
            }
