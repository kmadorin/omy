...
export async function GET(req: Request) {
	...
	for (const p of positions) {
		...
		const usdVal = new Decimal((p as unknown as { usdValue?: number }).usdValue ?? 0);
		if (!((p as unknown as { usdValue?: number }).usdValue === null)) {
	...
	const avgApy = totalAmount.eq(0) ? 0 : weightedApy.div(totalAmount).toNumber();
