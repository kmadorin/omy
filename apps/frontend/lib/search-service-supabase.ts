...
export async function getSearchResults(query: string): Promise<YieldResult[]> {
	try {
		...
		const { data: textToSqlData, error: textToSqlError } = await supabase.functions.invoke("text-to-sql", {
			body: { query },
		});

		if (textToSqlError) {
			throw new Error(`Error converting text to SQL: ${textToSqlError.message}`);
		}

		if (!textToSqlData.isRelevantToYield) {
			throw new Error(`Your query doesn't seem to be about yield opportunities. Please try a different query.`);
		}

		if (!textToSqlData.sql || textToSqlData.sql.trim() === "SELECT") {
			throw new Error(`No Yield data related to this query has been found. Please try rephrasing your question.`);
		}

		console.log(`textToSqlData: ${JSON.stringify(textToSqlData)}`);

		// Clean the SQL query before sending to DB
		let cleanSql = textToSqlData.sql.replace(/\\"/g, '"').replace(/;/g, "").trim();

		cleanSql = cleanSql.replace(/FROM\s+public\.\"YieldOpportunity\"/i, 'FROM "public"."YieldOpportunity"');
		cleanSql = cleanSql.replace(/FROM\s+\"public\"\.YieldOpportunity/i, 'FROM "public"."YieldOpportunity"');
		cleanSql = cleanSql.replace(/FROM\s+public\.YieldOpportunity/i, 'FROM "public"."YieldOpportunity"');
		...
		const { data: queryResults, error: queryError } = await supabase.rpc("execute_query", {
			sql_query: cleanSql,
		});
		...
		const typedResults = Array.isArray(queryResults) ? (queryResults as YieldResult[]) : [];
